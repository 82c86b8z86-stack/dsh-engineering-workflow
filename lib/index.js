...
/**
 * Compare the bundled preset tree against an installed target without
 * writing anything. One entry per bundled preset id:
 *   ok      — byte-identical with the source
 *   stale   — differs (per-file `differing` list, plus `extra` target-only files)
 *   missing — not installed at the target
 * @param sourceRoot - plugin-owned preset tree (bundled in the package).
 * @param targetRoot - dsh agent-presets discovery root.
 * @returns { id, status, differing?, extra? }[]
 */
export function verifyPresetTrees(sourceRoot, targetRoot) {
  const report = [];
  if (!existsSync(sourceRoot)) return report;
  for (const entry of readdirSync(sourceRoot)) {
    ...
    const sourceSet = new Set(filesUnder(source).map((file) => relative(source, file)));
    const targetSet = new Set(filesUnder(target).map((file) => relative(target, file)));
    const differing = [];
    for (const file of sourceSet) {
      const dest = join(target, file);
      if (!existsSync(dest) || !sameFile(join(source, file), dest)) differing.push(file);
    }
